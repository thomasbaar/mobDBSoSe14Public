 ...
 */
public class CNoteEditor extends Activity {
	// For logging and debugging purposes
	private static final String TAG = "CNoteEditor";

	/*
	 * Creates a projection that returns the note ID and the note contents.
	 */
	private static final String[] PROJECTION = new String[] {
			CNotePad.CNotes._ID, CNotePad.CNotes.COLUMN_NAME_TITLE,
	...
	private EditText lastModifiedText;
	...
	protected void onCreate(Bundle savedInstanceState) {
		 ...
		 */
		cursor = managedQuery(uri, // The URI that gets multiple notes from the
									// provider.
		...
		contentText = (EditText) findViewById(R.id.note);

		titleText = (EditText) findViewById(R.id.titleview);

		titleText.setEnabled(false);

		lastModifiedText = (EditText) findViewById(R.id.lastmodifiedview);
	...
	protected void onResume() {
		...
		if (cursor != null) {
			...
			int colContentIndex = cursor
					.getColumnIndex(CNotePad.CNotes.COLUMN_NAME_CONTENT);
			String content = cursor.getString(colContentIndex);
			contentText.setTextKeepState(content);

			int colTitleIndex = cursor
					.getColumnIndex(CNotePad.CNotes.COLUMN_NAME_TITLE);
			titleText.setTextKeepState(cursor.getString(colTitleIndex));

			int colModifiedIndex = cursor
					.getColumnIndex(CNotePad.CNotes.COLUMN_NAME_MODIFICATION_DATE);
			String currentTimeMillisString = cursor.getString(colModifiedIndex);
			long currentTimeMillesLong = Long.parseLong(currentTimeMillisString);
			Date lastModifiedDate = new Date(currentTimeMillesLong);
			
			lastModifiedText.setTextKeepState(String.format(res.getString(R.string.text_lastmodified) + ":\n" + lastModifiedDate.toString()));
			...
			contentText.setText(getText(R.string.error_message));
		}
	}
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		String currentNote = contentText.getText().toString();
	...
	private final void updateNote(String text, String title) {
		...
		values.put(CNotePad.CNotes.COLUMN_NAME_MODIFICATION_DATE,
		...
		if (state == STATE_INSERT) {
			...
			values.put(CNotePad.CNotes.COLUMN_NAME_TITLE, title);
		} else if (title != null) {
			// In the values map, sets the value of the title
			values.put(CNotePad.CNotes.COLUMN_NAME_TITLE, title);
		}
		...
		values.put(CNotePad.CNotes.COLUMN_NAME_CONTENT, text);
		 ...
		 */
		getContentResolver().update(uri, // The URI for the record to update.
	...
	public void onClickEditTitle(View v) {
		startActivity(new Intent(TitleEditor.EDIT_TITLE_ACTION, uri));
	}
